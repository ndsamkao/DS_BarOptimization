...
class OptimizeWarningBar:
    ...
    def level_to_action(self, level) -> Tuple:
    ...
    def simulate_data(self) -> pd.DataFrame:
        ...
        # charging_period:
        start_charging_index = data['time'].dt.time >= pd.to_datetime(f'{str(self.charging_period[0])}:00').time()
        end_charging_index = data['time'].dt.time <= pd.to_datetime(f'{str(self.charging_period[1])}:00').time()

        label = get_warning(data['load'] - data['solar'], threshold=self.threshold, contract_capacity=self.contract_capacity)
    ...
    def optimize_warning_bar(self, **kwargs: Any) -> float:
        ...
        if len(max_kw) >= 1: # 如果超約，計算超約的月份即可
            ...
            oc = OptimizeCapacity()
            before_result = oc.optimize_capacity(demand = max_kw, 
    ...
    def obj_fn(self, bar: np.array) -> float:
        ...
        bar_model = OptimizeWarningBar(df = self.df, 
                                       battery_capacity = self.battery_capacity, 
    ...
    def get_optimal_result(self, **kwargs: Any) -> pd.DataFrame:
        ...
        threshold_tmp = pd.Series([(list(i) + [self.safty_point]) for i in threshold_all[rnd_idx]], dtype=np.float)    
