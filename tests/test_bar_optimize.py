...
def test_optimize_bar(optimize_warning_bar_data):
    ...
    bar_model = OptimizeWarningBar(df = optimize_warning_bar_data, 
                 battery_capacity = 15, 
                 battery_initial = 0, 
    ...
    expected = pd.DataFrame({'new_fees': [24196.804, 24196.804, 24196.804], 
              'bar': [(0.83, 0.88, 0.91, 0.3), 
                      (0.81, 0.91, 0.98, 0.3), 
                      (0.81, 0.91, 0.95, 0.3)], 
              'old_fees': [24196.804, 24196.804, 24196.804], 
              "save": [0.0, 0.0, 0.0], 
              "judge": {False, False, False}}, index=[0,1, 2])
    
    assert all(solar_production_kw == solar_spec)
